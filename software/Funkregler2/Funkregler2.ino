/*****************************************************************
 ...
 19 Aug 2020     switching between loconet and SX via serial config (rev 0.40+)
...
#include "Display7.h"
#include "Loco.h"
#include "utils.h" // includes debug settings
...
bool initFromEEPROM()
{
	...
	String modeString = eep.readCCmode();
	if (modeString.length() == 0)
	{ // no value in EEPROM so far
		ccmode = CCMODE_SX;
		eep.writeCCmode(s_ccmode[ccmode]);
	}
	else
	{
		// search for value
		...
		{
			if (modeString.equalsIgnoreCase(s_ccmode[i]))
			{
				ccmode = i;
			}
		}
	...
	if ((cal_33 < 3000) || (cal_33 > 3500))
	{ //cal value makes no sense
		// not yet set
...
void sendAnnounceMessage()
{
	...
	if ((WiFi.status() == WL_CONNECTED) && (WiFi.RSSI() > -80) && (WiFi.RSSI() != 0))
	{
...
void addrClickEnd()
{
	...
	{ // new loco
		...
		sendRequestLoco(loco.getAddress()); // read state from central command bus
		mode = MODE_WAITING_FOR_RESPONSE;
		disp.dispCharacters('-', '0');
	}
	else
	{ // no change, just continue with stored value for current speed
...
void loop()
{
	...
	else if ((mode == MODE_ADDRESS_SELECTION) || (mode == MODE_FAST_ADDRESS_SELECTION))
	{
	...
	else if (mode == MODE_NORMAL)
	{
		...
		if ((millis() - updateTimer) >= 100)
		{
			...
			if (changedFlag == true)
			{						   // there was some user interaction
				userInteraction();	   // reset switch off timer
				sendAndDisplaySpeed(); // dispSpeed() will also be called.
				// reset changed flag
				changedFlag = false;
 ...
 *  and set new ccmode
 */
bool isValidCCMode(String s)
{
	for (int i = 0; i < N_CCMODE; i++)
	{
		if (s.equalsIgnoreCase(s_ccmode[i]))
		{
			ccmode = i;
			return true;
		}
...
void toggleConfig(void)
{
	...
	else
	{
		disp.dispCharacters('c', 'o'); // == (co)nfig
		if (!Serial)
		{ //  config only works with USB connected
...
{
	Serial.println(
		"CONFIGURATION - commands: exit, list, ssid=, pass=, cc=, locos=");
...
bool updateConfig(String line)
{
	...
	else if (line.startsWith("cc="))
	{
		...
		if (isValidCCMode(s))
		{
			...
			{
				Serial.println("could not write cc config");
			}
		}
		else
		{
			Serial.print("not a valid cc mode: ");
			Serial.println(s);
		}
	}
